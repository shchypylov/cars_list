...
const VehicleFilters = () => {
    ...
    useEffect(  () => {
        const getColors = async () => {
            const {colors} = await fetch('https://auto1-mock-server.herokuapp.com/api/colors', {
                method: 'GET',
                headers: {
    ...
    useEffect(  () => {
        const getManufacturers = async () => {
            const {manufacturers} = await fetch('https://auto1-mock-server.herokuapp.com/api/manufacturers', {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json'
                }
    ...
    return (
        <Form onSubmit={handleSubmit}>
            ...
            <Form.Group className="mb-3" controlId="vehicleManufacturer">
                <Form.Label>Manufacturer</Form.Label>
                <Form.Select name='manufacturer' aria-label="Default select example">
                    <option value="all">All manufacturers</option>
                    {manufacturers.map(manufacturer => (
                        <option key={manufacturer.toLowerCase()} value={manufacturer.toLowerCase()}>{manufacturer}</option>
                    ))}
                </Form.Select>
            </Form.Group>
